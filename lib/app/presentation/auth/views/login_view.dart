...
class AuthView1 extends GetView<LoginController> {
  AuthView1({Key? key}) : super(key: key);
...
class AuthView extends GetView<LoginController> {
  ...
  return Theme(
      ...
      child: Scaffold(appBar: AppBar(),body: body(context: context)),
    );
  }
...
// variables para el manejo de los indicadores de progreso  
  ...
  Widget build(BuildContext context) {
    ...
    // Obtenemos los valores
    darkMode = true;//Theme.of(context).brightness==Brightness.dark;
    screenSize = MediaQuery.of(context).size;
